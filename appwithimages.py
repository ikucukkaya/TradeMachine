...
def calculate_score_oguls_playground(player_name, week, data, last_n_column):
    """
    Calculates the score for a player based on the current week and selected data range.
    """
    player_data = data[data['Player_Name'] == player_name].iloc[0]
    last_n_score = player_data[last_n_column]
    projection = player_data['Projection']
    score = (((20 - week) * projection) / 20) + ((week * last_n_score) / 20)
    score = max(2, score)
    return score
...
def evaluate_trade_oguls_playground(data, team1_players, team2_players, team1_injury_adjustments, team2_injury_adjustments, team1_name, team2_name, df_last_n_days, df_rest_of_season, num_top_players, n_days):
    """
    Evaluates the trade between Team 1 and Team 2 using Last N Days data.
    """
    week = calculate_week()
    st.markdown(f"<h3 style='text-align: center;'>Current Week: {week}</h3>", unsafe_allow_html=True)

    # Determine the column to use ('Last14' or 'Last30')
    last_n_column = f"Last{n_days}"
    ...
    for idx, player in enumerate(team1_players):
        score = calculate_score_oguls_playground(player, week, data, last_n_column)
        injury_adjustment = team1_injury_adjustments[idx]
        score += injury_adjustment  # Apply the injury adjustment (-1 or -2)
        score = max(2.00, score)  # Ensure the score is at least 2.00 after injury adjustment
        player_data = data[data['Player_Name'] == player].iloc[0]
                ...
                with text_col:
                    if detail['player'] == 'Empty Slot':
                        ...
                        st.markdown(
                            f"<div style='font-size:16px; margin-top:12px;'>"
                            f"<strong>{detail['player']}</strong>{injury_note}<br>"
                            f"(Last {n_days} Days: {detail['last_n_score']}, Projection: {detail['projection']}, Score: {detail['score']:.2f})"
                            f"</div>",
                            unsafe_allow_html=True
                        )
    with col2:
    ...
    # Prepare player details for sharing
    team1_details_text = "\n".join(
        [f"{detail['player']}: Last {n_days} Days={detail['last_n_score']}, Projection={detail['projection']}, Score={detail['score']:.2f}, Adjustment={detail['injury_adjustment']}" for detail in team1_details]
    )
    team2_details_text = "\n".join(
        [f"{detail['player']}: Last {n_days} Days={detail['last_n_score']}, Projection={detail['projection']}, Score={detail['score']:.2f}, Adjustment={detail['injury_adjustment']}" for detail in team2_details]
    )

    # Create share message
    share_message = (
        f"Trade Evaluation (Oğul's Playground - Last {n_days} Days):\n\n"
        f"*{team1_name}* Total Score: *{team1_total:.2f}*\n"
        f"*{team2_name}* Total Score: *{team2_total:.2f}*\n"
        f"*Trade Ratio: {trade_ratio:.2f}*\n"
        f"*{trade_status_text}*\n\n"
    ...
    # Calculate Team Averages Before and After Trade for Last N Days
    team1_avg_before_last_n_days = calculate_team_averages(df_last_n_days, team1_players_current_normalized, num_top_players)
    team1_avg_after_last_n_days = calculate_team_averages(df_last_n_days, team1_players_after_normalized, num_top_players)

    team2_avg_before_last_n_days = calculate_team_averages(df_last_n_days, team2_players_current_normalized, num_top_players)
    team2_avg_after_last_n_days = calculate_team_averages(df_last_n_days, team2_players_after_normalized, num_top_players)
    ...
    st.markdown(f"<h3 style='text-align: center;'>Team Averages Before and After Trade (Last {n_days} Days)</h3>", unsafe_allow_html=True)
    ...
    # Display Last N Days Averages
    st.markdown(f"#### 📊 Last {n_days} Days Averages")

    col_team1, col_team2 = st.columns(2)

    with col_team1:
        st.markdown(f"### {team1_name}")
        df_team1_last_n_days = create_avg_dataframe(team1_name, team1_avg_before_last_n_days, team1_avg_after_last_n_days)
        styled_df_team1_last_n_days = format_team_avg_dataframe(df_team1_last_n_days)
        st.write(styled_df_team1_last_n_days)

    with col_team2:
        st.markdown(f"### {team2_name}")
        df_team2_last_n_days = create_avg_dataframe(team2_name, team2_avg_before_last_n_days, team2_avg_after_last_n_days)
        styled_df_team2_last_n_days = format_team_avg_dataframe(df_team2_last_n_days)
        st.write(styled_df_team2_last_n_days)
    ...
    def df_to_text(df, team_name, title):
        ...
        - df: DataFrame containing the team averages (transposed)
        - team_name: Name of the team
        - title: Title for the table (e.g., "Last 14 Days Averages")
    ...
    # Generate text representations of the tables
    team1_last_n_days_avg_text = df_to_text(df_team1_last_n_days, team1_name, f"Last {n_days} Days Averages")
    team2_last_n_days_avg_text = df_to_text(df_team2_last_n_days, team2_name, f"Last {n_days} Days Averages")
    team1_projection_avg_text = df_to_text(df_team1_projection, team1_name, "Rest of Season Projections")
    team2_projection_avg_text = df_to_text(df_team2_projection, team2_name, "Rest of Season Projections")
    ...
    share_message = (
        ...
        f"--- {team2_name} Player Details ---\n{team2_details_text}\n\n"
        f"--- Team Averages Before and After Trade ---\n\n"
        f"{team1_last_n_days_avg_text}\n\n"
        f"{team2_last_n_days_avg_text}\n\n"
        f"{team1_projection_avg_text}\n\n"
        f"{team2_projection_avg_text}\n"
    )
...
@st.cache_data
def load_last_n_days_data(n_days):
    """
    Loads and processes data for the last N days.
    """
    if n_days == 14:
        file_name = "Last_14_days_of_the_2024-25_NBA_Regular_Season_Updated_daily.xlsx"
    elif n_days == 30:
        file_name = "Last_30_days_of_the_2024-25_NBA_Regular_Season_Updated_daily.xlsx"
    else:
        st.error("Invalid number of days selected.")
        return pd.DataFrame()
    file_path = os.path.join(data_dir, file_name)
    try:
        df_last_n_days = pd.read_excel(file_path)
        # Select required columns including 'R#'
        required_columns = ['PLAYER', 'R#', 'FG%', 'FT%', '3PM', 'PTS', 'TREB', 'AST', 'STL', 'BLK', 'TO', 'FGA', 'FTA']
        missing_cols = set(required_columns) - set(df_last_n_days.columns)
        if missing_cols:
            st.error(f"{file_name} is missing columns: {', '.join(missing_cols)}")
            df_last_n_days = pd.DataFrame()
        else:
            df_last_n_days = df_last_n_days[required_columns].copy()
            # Rename 'PLAYER' to 'Player_Name'
            df_last_n_days = df_last_n_days.rename(columns={'PLAYER': 'Player_Name'})
            # Ensure numerical columns are float, except for FGA and FTA which are strings
            numerical_cols = ['3PM', 'PTS', 'TREB', 'AST', 'STL', 'BLK', 'TO', 'R#']
            for col in numerical_cols:
                df_last_n_days[col] = pd.to_numeric(df_last_n_days[col], errors='coerce')
            # Remove existing 'Rank' column if present
            if 'Rank' in df_last_n_days.columns:
                df_last_n_days.drop(columns=['Rank'], inplace=True)
            # Reset index and rename 'index' to 'Rank'
            df_last_n_days.reset_index(inplace=True)
            df_last_n_days.rename(columns={'index': 'Rank'}, inplace=True)
            df_last_n_days['Rank'] = df_last_n_days['Rank'] + 1
            # Reorder columns to have 'Rank' first
            columns_order = ['Rank', 'R#', 'Player_Name', 'FG%', 'FT%', '3PM', 'PTS', 'TREB', 'AST', 'STL', 'BLK', 'TO', 'FGA', 'FTA']
            df_last_n_days = df_last_n_days[columns_order]
        return df_last_n_days
    except Exception as e:
        st.error(f"Failed to load data for the last {n_days} days: {e}")
        return pd.DataFrame()
...
def main():
    ...
    # ------------------- Load Last N Days Data -------------------
    # We will load these when needed in the Oğul's Playground tab
    ...
    # ------------------- Create Tabs -------------------
    tab1, tab2, tab3, tab4 = st.tabs(["Trade Evaluation", "Oğul's Playground", "Player Scores Analysis", "Team Scores Analysis"])
    ...
    # ------------------- Oğul's Playground Tab -------------------
    with tab2:
        st.markdown("<h3 style='text-align: center;'>Oğul's Playground</h3>", unsafe_allow_html=True)
        
        n_days = st.radio(
            "Select Data Range for Evaluation:",
            options=[14, 30],
            format_func=lambda x: f"Last {x} Days"
        )
        
        # Load the appropriate data
        df_last_n_days = load_last_n_days_data(n_days)
        
        # Normalize player names in last N days data
        df_last_n_days['Player_Name_Normalized'] = df_last_n_days['Player_Name'].apply(normalize_player_name)
        
        # Merge 'Takım' column into last N days data
        df_last_n_days = pd.merge(df_last_n_days, data[['Player_Name_Normalized', 'Takım']], on='Player_Name_Normalized', how='left')
        
        # ------------------- Team Selection -------------------
        # The rest of the code is similar to the Trade Evaluation tab but using the new evaluate_trade_oguls_playground function
        # ------------------- Team Selection -------------------
        st.markdown(f"<h3 style='text-align: center;'>Trade Evaluation (Last {n_days} Days)</h3>", unsafe_allow_html=True)
        ...
        with col_player1:
            ...
            team1_selected = st.multiselect(
                f"Select Players from {team1}",
                options=team1_players_available,
                key="ogul_team1_selected_players"
            )
        ...
        with col_player2:
            ...
            team2_selected = st.multiselect(
                f"Select Players from {team2}",
                options=team2_players_available,
                key="ogul_team2_selected_players"
            )
                    ...
                    for idx, player in enumerate(team1_selected):
                        ...
                        with col_status:
                            injury_status = st.selectbox(
                                "Injury Status",
                                options=list(injury_options.keys()),
                                key=f"ogul_team1_{player}_injury_status"
                            )
                        # Get the adjustment from the dictionary
                        injury_adjustment = injury_options[injury_status]
                        team1_injury_adjustments.append(injury_adjustment)
                    ...
                    for idx, player in enumerate(team2_selected):
                        ...
                        with col_status:
                            injury_status = st.selectbox(
                                "Injury Status",
                                options=list(injury_options.keys()),
                                key=f"ogul_team2_{player}_injury_status"
                            )
                        # Get the adjustment from the dictionary
                        injury_adjustment = injury_options[injury_status]
                        team2_injury_adjustments.append(injury_adjustment)
        ...
        num_top_players = st.number_input(
            ...
            value=15,      # Default value
            step=1,
            key="ogul_num_top_players"
        )

        # Center the Evaluate Trade button using columns
        col_center = st.columns([1, 0.275, 1])
        with col_center[1]:
            submitted = st.button("📈 Evaluate Trade", key="ogul_evaluate_trade_button")
        ...
        if submitted:
            ...
            if duplicate_players:
                ...
                if not team1_selected and not team2_selected:
                    ...
                    evaluate_trade_oguls_playground(
                        data,
                        team1_selected,
                        team2_selected,
                        team1_injury_adjustments,
